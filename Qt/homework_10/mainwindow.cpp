...
{
    ui->tb_result->append("Место на сервере: " + QString::number(freeSpace));
}
...
void MainWindow::DisplayStat(StatServer stat)
{
    ui->tb_result->append("Статистика сервера \n   Принято байт: " + QString::number(stat.incBytes) +
                          ...
                          "\n   Время работы сервера, сек: " + QString::number(stat.workTime) +
...
void MainWindow::DisplaySuccess(uint16_t typeMess)
{
    switch (typeMess) {
    case CLEAR_DATA: {
        ui->tb_result->append("Данные удалены с сервера");
    }
    default:
        break;
    }
